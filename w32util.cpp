...
WinDLL::WinDLL(const wchar_t *_name)
:	name(_name), mustfree(false), handle(0)
{
	handle = GetModuleHandleW(name);
	if (!handle)
		mustfree = (handle = LoadLibraryW(name)) != 0;
}

WinDLL::~WinDLL(){ if (mustfree) FreeLibrary(handle); }

WinDLL winShell32(L"shell32");
