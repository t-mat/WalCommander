...
//	if (wParam & MK_XBUTTON2) bf |= MB_X2;
	...
	cevent_mouse ev(type, cpoint(xPos, yPos), button, bf, km, delta);
...
static bool KeyEvent(int type, Win *win, WPARAM wParam, LPARAM lParam)
{
	...
	if (win->IsEnabled() && win->EventKey(&ev)) return true;
	
	return win->DoKeyPost(&ev);
}
...
//	break;
		...
		//!еле нашел, почему мышью активируются окна когда их не просят
		case WM_MOUSEACTIVATE:
                  if ((win->whint & Win::WH_CLICKFOCUS) == 0) return MA_NOACTIVATE;
                  break;
		...
		case WM_MOUSEWHEEL: 
			{
				...
				return MouseEvent(EV_MOUSE_WHEEL, 0, win, wParam, lParam);
			}
...
GC::GC(Win*win)
:	handle(
			::GetDC(win ? win->GetID(): 0) //!!! :)
		   //::GetWindowDC(win ? win->GetID(): 0) //эта функция создает хреновый dc, он позволяет на дочерних окнах рисовать :( 
		   ), 
	needDelete(true),
	savedPen(0),
	savedBrush(0),
...
//FW_DONTCARE 0 
T FW_THIN 100 
E FW_EXTRALIGHT 200,  FW_ULTRALIGHT 200 
L FW_LIGHT 300 
N FW_NORMAL 400, FW_REGULAR 400 
M FW_MEDIUM 500 
S FW_SEMIBOLD 600, FW_DEMIBOLD 600 
B FW_BOLD 700 
U FW_EXTRABOLD 800, FW_ULTRABOLD 800 
H FW_HEAVY 900, FW_BLACK 900 
*/
	...
	case FW_THIN:		uri.append('T'); break;
	case FW_EXTRALIGHT:	uri.append('E'); break;
	case FW_MEDIUM:		uri.append('M'); break;
	case FW_SEMIBOLD:	uri.append('S'); break;
	case FW_EXTRABOLD:	uri.append('U'); break;
	case FW_HEAVY:		uri.append('H'); break;
	};
...
void cfont::UriToLogFont(LOGFONT *plf, const char*uri)
{
	...
	if (*s == '-') 
	{
		...
		case 'T': plf->lfWeight = FW_THIN;	break;
		case 'E': plf->lfWeight = FW_EXTRALIGHT; break;
		case 'M': plf->lfWeight = FW_MEDIUM; break;
		case 'S': plf->lfWeight = FW_SEMIBOLD; break;
		case 'U': plf->lfWeight = FW_EXTRABOLD; break;
		case 'H': plf->lfWeight = FW_HEAVY; break;
